...
2. 가장 최근에 발생한 6번의 식사 기록
   ...
   - 각 기록에는 섭취한 음식, 영양소 섭취량, 감정(mood), 에너지 수준(energy)이 포함되어 있다
3. 기분 안정과 음식의 관계에 대한 참고 문서(reference_docs)

판단 원칙:
- 최근 6번의 식사 기록을 시간 순서대로 고려하여,
  최근에 반복적으로 나타나는 감정 및 에너지 상태의 경향을 파악한다.
- 식사 기록을 통해 영양소의 과다 또는 부족 패턴을 우선적으로 판단한다.
- 기분을 과도하게 자극하지 않고 안정적으로 유지하는 방향을 가장 중요한 기준으로 삼는다.
- reference_docs는 메뉴 선택 시 중요한 참고 근거로 최대한 활용한다.
- reference_docs에 직접 등장하지 않는 음식이라도,
  위 조건(영양 균형, 기분 안정)에 잘 부합한다면 선택할 수 있다.
  단, 실제로 있는 음식이어야 한다.
- 여러 후보 중, 현재 사용자 상태에 가장 적합한 메뉴 하나만 최종 선택한다.
- 특정 음식 하나가 지속적으로 추천되지 않도록, 식품 다양성을 함께 고려한다.

출력 규칙:
- 설명하지 않는다.
- 주어, 서술어 없이 메뉴 이름만 출력한다.
- 반드시 하나의 메뉴만 선택한다.
- JSON 형식만 출력한다.
...
# data = {
#   "user_id": "0000000011",	
#   "recommend_info": {"kcal": 2019.61,	"carbohydrate": 101,	"protein":252 ,	"fat":151 },
#   "meal_data": [
#     {
#         "rgs_dt": "20251120",
#         "time_slot": "L",
