...
def _norm_mood_val(x: Any) -> str:
    s = str(x).strip().lower()
    if s in ("pos", "positive"): return "pos"
    if s in ("neu", "neutral"):  return "neu"
    if s in ("neg", "negative"): return "neg"
    if s == "pos ": return "pos"
    # artifacts 케이스: Pos/Neu/Neg
    if s == "pos": return "pos"
    if s == "neu": return "neu"
    if s == "neg": return "neg"
    return s
...
def _norm_energy_val(x: Any) -> str:
    s = str(x).strip().lower()
    # artifacts 케이스: High/Med/Low
    if s in ("low",): return "low"
    if s in ("med", "mid"): return "med"
    # constants가 'hig'를 쓰는 경우가 많아서 high -> hig로 통일
    if s in ("high", "hig"): return "hig"
    return s
...
def recommend_and_commit(
    *,
    ...
    rec_df = recommend_phase1_2plus1(
        ...
        mood=mood_key,               # ✅ Pos/Neu/Neg
        energy=energy_key,           # ✅ Low/Med/High
        cfg=phase1_cfg,
    ...
    # ✅ Phase1 결과가 ERROR여도 컬럼 보강(후속 단계 안정화)
    ...
    if clustered is None or clustered.empty:
        # cluster가 없다면 p_stable 계산 불가 → default
